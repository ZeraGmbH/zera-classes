...
void ScpiDocsHtmlGenerator::createScpiDocHtmls(QString modmanConfigFile, QString zenuxRelease, QString htmlDirPath, QString xmlDirPath, QString sessionMapPath)
{
    ...
    //copy css files
    QDir sourceCssDir(QStringLiteral(SCPI_DOC_SOURCE_PATH));
    QDir destCssDir(htmlDirPath);
    for (QString &fileName: sourceCssDir.entryList(QStringList({"*.css"}), QDir::Files)) {
