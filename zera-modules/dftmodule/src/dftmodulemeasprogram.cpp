...
void cDftModuleMeasProgram::pcbserverConnect()
{
    ...
    for (int i = 0; i < channelInfoReadList.count(); i++)
    {
        ...
        connect(pcbClient, &Zera::Proxy::ProxyClient::connected, this, &cDftModuleMeasProgram::monitorConnection); // here we wait until all connections are established
...
{
    Zera::Proxy::Proxy::getInstance()->releaseConnection(m_pDspClient); // no async. messages anymore
