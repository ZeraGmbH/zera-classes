...
void cReferenceModuleMeasProgram::setDspVarList()
{
    ...
    m_pTmpDataDsp->addVarItem( new cDspVar("CHXDC",m_ChannelList.count(), DSPDATA::vDspTemp));
    ...
    m_pActualValuesDSP->addVarItem( new cDspVar("CHXDCF",m_ChannelList.count(), DSPDATA::vDspResult));
...
void cReferenceModuleMeasProgram::setDspCmdList()
{
    ...
    for (int i = 0; i < m_ChannelList.count(); i++)
    {
        ...
        m_pDSPInterFace->addCycListItem( s = QString("INTEGRAL(%1,MEASSIGNAL,CHXDC+%2)").arg(m_nSamples).arg(i));
    }

    // and filter them
    m_pDSPInterFace->addCycListItem( s = QString("AVERAGE1(%1,CHXDC,FILTER)").arg(m_ChannelList.count())); // we add results to filter
    ...
    m_pDSPInterFace->addCycListItem( s = "STARTCHAIN(0,1,0x0102)");
        ...
        m_pDSPInterFace->addCycListItem( s = QString("CMPAVERAGE1(%1,FILTER,CHXDCF)").arg(m_ChannelList.count()) );
...
{
    m_MsgNrCmdList[m_pRMInterface->rmIdent(QString("ReferenceModule%1").arg(m_pModule->getModuleNr()))] = sendrmident;
}
