...
cDftModuleMeasProgram::cDftModuleMeasProgram(cDftModule* module) :
    ...
    m_pModule(module)
{
    m_dspInterface = m_pModule->getServiceInterfaceFactory()->createDspInterfaceDft(
        ...
        m_pModule->getConfigData()->m_nDftOrder);
...
void cDftModuleMeasProgram::generateVeinInterface()
{
    ...
    for (int i = 0; i < m_pModule->getConfigData()->m_valueChannelList.count(); i++) {
        ...
        if (measChannels.count() == 1) { // in this case we have phase,neutral value
            ...
            pActvalue = componentCreator.createComponent(QString("ACT_DFTPN%1").arg(phaseNeutralValueCount+1),
            ...
            if(!isConfiguredForDc()) {
                pActvalue = componentCreator.createComponent(QString("ACT_POL_DFTPN%1").arg(phaseNeutralValueCount+1),
                                                             channelDescriptionPolar,
                                                             channelName, channelUnit,
                                                             "MEASURE", scpiChannelName, SCPI::isCmdwP);
                m_veinPolarValue.append(pActvalue);
                m_pModule->m_veinComponentsWithMetaAndScpi.append(pActvalue);
            }
            else {
                const QString scpiChannelNameDc = scpiChannelName.replace("REF", "R") + "DC"; // SCPI short names suck!!!
                pActvalue = componentCreator.createComponent(QString("ACT_DC%1").arg(phaseNeutralValueCount+1), "DC actual value",
                                                             channelName, channelUnit,
                                                             "MEASURE", scpiChannelNameDc, SCPI::isCmdwP);
            ...
            pActvalue = componentCreator.createComponent(QString("ACT_DFTPP%1").arg(phasePhaseValueCount+1),
                                                         "Actual value phase/phase / cartesian format: re,im",
                                                         channelName, channelUnit,
                                                         "MEASURE", channelName, SCPI::isCmdwP);
            m_veinActValueList.append(pActvalue);
            ...
            if(!isConfiguredForDc()) {
                pActvalue = componentCreator.createComponent(QString("ACT_POL_DFTPP%1").arg(phasePhaseValueCount+1),
                                                             "Actual value phase/phase / polar format: abs,rad[0,2π],deg[0,360]",
                                                             channelName, channelUnit,
                                                             "MEASURE", scpiChannelName, SCPI::isCmdwP);
    ...
    if (!isConfiguredForDc()) {
        m_pRFieldActualValue = componentCreator.createComponent("ACT_RFIELD",
                                                                "Phase sequence",
                                                                "", "",
                                                                "MEASURE", "RFIELD", SCPI::isCmdwP);
...
void cDftModuleMeasProgram::setDspVarList()
{
    ...
    m_pActualValuesDSP->addDspVar("VALXDFTF", 2*m_veinActValueList.count());
...
void cDftModuleMeasProgram::setDspCmdList()
{
    ...
    for (int i = 0; i < m_pModule->getConfigData()->m_valueChannelList.count(); i++)
    {
        ...
        m_dspInterface->addCycListItem(QString("DFT(%1,TMP_SAMPLES_SINGLE_1,VALXDFT+%2)").arg(m_pModule->getConfigData()->m_nDftOrder).arg(2*i));
    }
...
void cDftModuleMeasProgram::setRefChannelValidator()
{
    ...
    for(const QString &channelMName : channelMNames) {
        if(m_pModule->getConfigData()->m_valueChannelList.contains(channelMName)) {
...
{
    return m_pModule->getConfigData()->m_nDftOrder == 0;
}

void cDftModuleMeasProgram::calcPhaseSequence(const QVector<float> *actualValues)
{
    if (!isConfiguredForDc()) {
        int rfieldChannelCount = m_pModule->getConfigData()->m_rfieldChannelList.length();
        if (rfieldChannelCount == 3) {
            QList<double> angles;
            for (int rfieldChannel = 0; rfieldChannel < rfieldChannelCount; rfieldChannel++) {
                int rfieldChannelIdx = m_pModule->getConfigData()->m_valueChannelList.indexOf(m_pModule->getConfigData()->m_rfieldChannelList.at(rfieldChannel));
...
void cDftModuleMeasProgram::dataReadDSP()
{
    if (m_bActive) {
        ...
        if (isConfiguredForDc()) {
            ...
            if (m_pModule->getConfigData()->m_bRefChannelOn)
...
void cDftModuleMeasProgram::turnVectorsToRefChannel()
{
    ...
    const cDftModuleConfigData *configData = m_pModule->getConfigData();
    ...
    for (int i = 0; i < configData->m_valueChannelList.count(); i++) {
        QString key;
        key = configData->m_valueChannelList.at(i);
    ...
    for (int i = 0; i < configData->m_valueChannelList.count(); i++) {
        QString key = configData->m_valueChannelList.at(i);
