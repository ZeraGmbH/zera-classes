...
void ScpiModelParameters::setupScpi(cSCPIInterface *scpiInterface)
{
    ...
    const ScpiVeinParser::ScpiParseInfo allParamInfos = moduleInterfaces.getParamInfo();

    for (auto iter = allParamInfos.constBegin(); iter != allParamInfos.constEnd(); ++iter) {
        const ScpiVeinParser::ScpiComponentParseInfo &params = iter.value();
...
void ScpiModelParameters::addSCPICommand(cSCPIInterface *scpiInterface, const cSCPICmdInfoPtr &scpiCmdInfo)
{
    ...
    ScpiBaseDelegatePtr delegate = std::make_shared<ScpiDelegateParameter>(ScpiDelegateParameter::Params{cmdParent, cmdNode, scpiCmdInfo->scpiQueryCmdFlags, entityId, componentName, m_scpiModule});
