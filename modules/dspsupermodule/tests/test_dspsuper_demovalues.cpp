#include "test_dspsuper_demovalues.h"
#include "demovaluesdspdspsuper.h"
#include <QTest>

QTEST_MAIN(test_dspsuper_demovalues)

const QStringList test_dspsuper_demovalues::mtChannelLayout = QStringList() << "m0" << "m1" << "m2" << "m3" << "m4" << "m5" << "m6" << "m7";

void test_dspsuper_demovalues::addVoltage()
{
    constexpr int maxPeriods = 10;
    constexpr int measPeriods = 5;
    DemoValuesDspDspSuper values(mtChannelLayout, maxPeriods, measPeriods);
...
float test_dspsuper_demovalues::getDspPeriodValue(const QString &channelMName,
                                                       int period,
...
float test_dspsuper_demovalues::getDspAvgValue(const QString &channelMName,
                                                    int maxPeriods,
                                                    const QVector<float> &dspValues)
{
    int channelCount = mtChannelLayout.count();
