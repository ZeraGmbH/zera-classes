...
static constexpr int countResultsPerPeriod = 2; // pair of global period / ms timer value
...
void DspSuperModuleMeasProgram::setDspVarList()
{
    ...
    tmpDspVarGroup->addDspVar("SYSTIME", 1, dspDataTypeInt);
    ...
    tmpDspVarGroup->addDspVar("TIMESTAMPS_PERIOD_WORK", getConfData()->m_maxPeriods * countResultsPerPeriod); // double buffer -> TIMESTAMPS_PERIOD

    // results
    m_pActualValuesDSP = m_dspInterface->createVariableGroup("ActualValues");
    m_pActualValuesDSP->addDspVar("TIMESTAMPS_PERIOD", getConfData()->m_maxPeriods  * countResultsPerPeriod);
}
...
void DspSuperModuleMeasProgram::setDspCmdList()
{
    ...
    // * DspSuperModule' asks zdsp1s to handle it's interrupts FIRST. So other modules using timestamps
    ...
    m_dspInterface->addCycListItem("STARTCHAIN(1,1,0x0101)");
        ...
        m_dspInterface->addCycListItem(QString("SETVAL(RESULT_BUFF_IDX,TIMESTAMPS_PERIOD_WORK)"));
    ...
    m_dspInterface->addCycListItem("GETSTIME(SYSTIME)");
    m_dspInterface->addCycListItem("XCOPYMEM(1,SYSTIME,RESULT_BUFF_IDX)");

    // increment local & global period count
    m_dspInterface->addCycListItem("INC(PERIODCURR)");
    m_dspInterface->addCycListItem("INC(GLOBALPERIODCOUNT)");
    ...
    m_dspInterface->addCycListItem("STARTCHAIN(0,1,0x0102)");
        ...
        m_dspInterface->addCycListItem(QString("SETVAL(RESULT_BUFF_IDX,TIMESTAMPS_PERIOD_WORK)"));
        m_dspInterface->addCycListItem(QString("COPYMEM(%1,TIMESTAMPS_PERIOD_WORK,TIMESTAMPS_PERIOD)").arg(getConfData()->m_maxPeriods*countResultsPerPeriod));
