...
cPower3ModuleConfiguration::cPower3ModuleConfiguration(const QByteArray &xmlString)
{
...
QByteArray cPower3ModuleConfiguration::exportConfiguration() const
{
    return m_pXMLReader->getXMLConfig().toUtf8();
}


cPower3ModuleConfigData *cPower3ModuleConfiguration::getConfigData()
{
    return &m_configData;
}
...
void cPower3ModuleConfiguration::configXMLInfo(const QString &key)
{
    if (m_ConfigXMLMap.contains(key)) {
        ...
        case setSystemCount:
        {
            ...
            m_configData.m_nPowerSystemCount = m_pXMLReader->getValue(key).toInt(&ok);
            ...
            for (int i = 0; i < m_configData.m_nPowerSystemCount; i++)
            {
                ...
                m_configData.m_powerSystemConfigList.append(psc);
                m_configData.m_powerChannelList.append("");
            }
            break;
        }
        default:
            ...
            if ((cmd >= setMeasSystem1) && (cmd < setMeasSystem1 + 12))
            {
                ...
                powersystemconfiguration psc = m_configData.m_powerSystemConfigList.at(cmd);
                psc.m_sInputU = measChannels.at(0);
                psc.m_sInputI = measChannels.at(1);

                m_configData.m_powerSystemConfigList.replace(cmd, psc);
                m_configData.m_powerChannelList.replace(cmd, measChannels.at(2));
            }
        }
        m_bConfigError |= !ok;
    }
