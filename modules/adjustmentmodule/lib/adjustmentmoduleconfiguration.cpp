...
cAdjustmentModuleConfiguration::cAdjustmentModuleConfiguration(const QByteArray& xmlString)
{
...
QByteArray cAdjustmentModuleConfiguration::exportConfiguration() const
{
    return m_pXMLReader->getXMLConfig().toUtf8();
}

cAdjustmentModuleConfigData *cAdjustmentModuleConfiguration::getConfigData()
{
    return &m_configData;
}
...
void cAdjustmentModuleConfiguration::configXMLInfo(const QString &key)
{
    if (m_ConfigXMLMap.contains(key)) {
        ...
        case setAngleReferenceEntity:
            m_configData.m_ReferenceAngle.m_nEntity = m_pXMLReader->getValue(key).toInt(&ok);
            break;
        case setAngleReferenceComponent:
            m_configData.m_ReferenceAngle.m_sComponent = m_pXMLReader->getValue(key);
            break;
        case setFrequencyReferenceEntity:
            m_configData.m_ReferenceFrequency.m_nEntity = m_pXMLReader->getValue(key).toInt(&ok);
            break;
        case setFrequencyReferenceComponent:
            m_configData.m_ReferenceFrequency.m_sComponent = m_pXMLReader->getValue(key);
            break;

        case setChannelCount:
        {
            int nr;
            nr = m_configData.m_nAdjustmentChannelCount = m_pXMLReader->getValue(key).toInt(&ok);
            for (int i = 0; i < nr; i++)
            {
        ...
        default:
            ...
            if ((cmd >= setChn1Chn) && (cmd < setChn1Chn + 32)) {
                ...
                m_configData.m_AdjChannelList.append(chn);
                m_configData.m_AdjChannelInfoHash[chn] = new cAdjChannelInfo();
            }

            else if ((cmd >= setChn1AcRmsValueAvail) && (cmd < setChn1AcRmsValueAvail + 32)) {
                cmd -= setChn1AcRmsValueAvail;
                QString chn = m_configData.m_AdjChannelList.at(cmd);
                cAdjChannelInfo* adjChannelInfo = m_configData.m_AdjChannelInfoHash[chn];
