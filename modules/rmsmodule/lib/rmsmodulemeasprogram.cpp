...
void cRmsModuleMeasProgram::setDspVarList()
{
    ...
    m_pTmpDataDsp->addVarItem( new DspVariable("VALXRMS",m_veinActValueList.count(), DSPDATA::vDspTemp));
    m_pTmpDataDsp->addVarItem( new DspVariable("FILTER",2*m_veinActValueList.count(),DSPDATA::vDspTemp));
    ...
    m_pActualValuesDSP->addVarItem( new DspVariable("VALXRMSF",m_veinActValueList.count(), DSPDATA::vDspResult));
...
void cRmsModuleMeasProgram::newIntegrationtime(QVariant ti)
{
    ...
    if (getConfData()->m_sIntegrationMode == "time") {
        if (getConfData()->m_bmovingWindow)
            ...
            m_pParameterDSP->setVarData(QString("TIPAR:%1;TISTART:%2;").arg(getConfData()->m_fMeasIntervalTime.m_fValue*1000).arg(0));
...
void cRmsModuleMeasProgram::newIntegrationPeriod(QVariant period)
{
    ...
    if(getConfData()->m_sIntegrationMode == "period") {
        m_pParameterDSP->setVarData(QString("TIPAR:%1;TISTART:%2;").arg(getConfData()->m_nMeasIntervalPeriod.m_nValue).arg(0));
        m_MsgNrCmdList[m_dspInterface->dspMemoryWrite(m_pParameterDSP)] = writeparameter;
    }
    emit m_pModule->parameterChanged();
}
