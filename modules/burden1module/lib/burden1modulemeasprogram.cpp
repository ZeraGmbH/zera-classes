...
cBurden1ModuleMeasProgram::cBurden1ModuleMeasProgram(cBurden1Module* module) :
...
{
    const cBurden1ModuleConfigData *configData = m_pModule->getConfigData();
    for (int i = 0; i < configData->m_nBurdenSystemCount; i++) {
    ...
    m_pNominalRangeParameter = new VfModuleParameter(m_pModule->getEntityId(), m_pModule->getValidatorEventSystem(),
                                                     ...
                                                     QVariant(configData->nominalRange.m_fValue));
    m_pNominalRangeParameter->setUnit(configData->m_Unit);
    ...
    m_pNominalBurdenParameter = new VfModuleParameter(m_pModule->getEntityId(), m_pModule->getValidatorEventSystem(),
                                                         ...
                                                         QVariant(configData->nominalBurden.m_fValue));
    ...
    m_pWireLengthParameter = new VfModuleParameter(m_pModule->getEntityId(), m_pModule->getValidatorEventSystem(),
                                                   ...
                                                   QVariant(configData->wireLength.m_fValue));
    ...
    m_pWireCrosssectionParameter = new VfModuleParameter(m_pModule->getEntityId(), m_pModule->getValidatorEventSystem(),
                                                            ...
                                                            QVariant(configData->wireCrosssection.m_fValue));
    ...
    m_pBRSCountInfo = new VfModuleMetaData(QString("BRSCount"), QVariant(configData->m_nBurdenSystemCount));
...
void cBurden1ModuleMeasProgram::searchActualValues()
{
    ...
    const cBurden1ModuleConfigData *configData = m_pModule->getConfigData();
    m_dftSignal = storageDb->findComponent(configData->m_nModuleId, "SIG_Measuring");
    if (m_dftSignal) {
        ...
        for (int i = 0; i < configData->m_nBurdenSystemCount; i++) {
            VeinStorage::AbstractComponentPtr inputVectorU =
                storageDb->findComponent(configData->m_nModuleId, configData->m_BurdenSystemConfigList.at(i).m_sInputVoltageVector);
            VeinStorage::AbstractComponentPtr inputVectorI =
                storageDb->findComponent(configData->m_nModuleId, configData->m_BurdenSystemConfigList.at(i).m_sInputCurrentVector);
...
{
    m_pModule->getConfigData()->nominalRange.m_fValue = nr.toFloat();
    setParameters();
...
{
    m_pModule->getConfigData()->nominalBurden.m_fValue = nb.toFloat();
    setParameters();
    emit m_pModule->parameterChanged();
}
...
{
    m_pModule->getConfigData()->wireLength.m_fValue = wl.toFloat();
    setParameters();
    emit m_pModule->parameterChanged();
}
...
{
    m_pModule->getConfigData()->wireCrosssection.m_fValue = wc.toFloat();
    setParameters();
    emit m_pModule->parameterChanged();
}
...
void cBurden1ModuleMeasProgram::setParameters()
{
    ...
    for (int i = 0; i < m_Burden1MeasDelegateList.count(); i++) {
        ...
        tmd->setNominalBurden(configData->nominalBurden.m_fValue);
