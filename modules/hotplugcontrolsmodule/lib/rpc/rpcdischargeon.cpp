...
RpcDischargeOn::RpcDischargeOn(Zera::PcbInterfacePtr pcbInterface,
    ...
    VfCpp::VfCppRpcSimplified(eventSystem, entityId,
                              VfCpp::VfCppRpcSignature::createRpcSignature(
                                  "RPC_dischargeOn", VfCpp::VfCppRpcSignature::RPCParams({{"p_channelName", "QString"}}))),
...
void RpcDischargeOn::callRPCFunction(const QUuid &callId, const QVariantMap &parameters)
{
    ...
    TaskTemplatePtr taskSwitch = TaskEmobDischarge::create(m_pcbInterface, channelMName, true, TRANSACTION_TIMEOUT);
    TaskRpcTransactionWrapperPtr taskRpcWrapper = TaskRpcTaskWrapper::create(std::move(taskSwitch), callId);
    connect(taskRpcWrapper.get(), &TaskRpcTaskWrapper::sigRpcFinished, this, &RpcDischargeOn::onRpcTaskFinish);
    m_taskQueue->addSub(std::move(taskRpcWrapper));
}

void RpcDischargeOn::onRpcTaskFinish(bool ok, QUuid rpcCallId)
{
    if(ok)
